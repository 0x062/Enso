...
import { sendTelegramReport } from './telegramReporter.js'; // Sesuaikan path jika berbeda
...
async function getUserInfo(zealyUserId, proxy = null, retryCount = 0) {
    ...
    try {
        ...
        return {
            ...
            xp: response.data.xp || 0, // Pastikan ini mengambil nilai XP yang benar
        };
    } catch (err) {
        ...
        return {
            ...
            xp: 0, // Mengembalikan 0 atau N/A jika gagal
        };
    }
}
...
// --- Fungsi processAccounts yang telah dimodifikasi ---
async function processAccounts(accounts, messages, accountProxies, noType) {
    ...
    // + Array untuk menyimpan data laporan Telegram setiap akun
    const reportData = [];
    ...
    for (let i = 0; i < accounts.length; i++) {
        ...
        let currentAccountXp = 'N/A'; // + Default value untuk XP di laporan Telegram
        let accountStatusText = 'Sukses Penuh'; // + Default status untuk laporan Telegram
        ...
        try {
            ...
            for (let j = 0; j < DEFIDEX_LIMIT; j++) {
                ...
                if (success) {
                    ...
                    failedDexes++;
                    partialFailure = true;
                    if (!success && j === 0) break; // Jika gagal di DeFiDex pertama, jangan lanjutkan DeFiDex lain untuk akun ini
                }
                await sleep(1000);
            }
            console.log(chalk.yellow(' ┊ └──'));
            ...
            if (campaigns.length === 0) {
                ...
                console.log(chalk.yellow(' ┊ └──'));
                partialFailure = true; // + tandai sebagai parsial jika tidak bisa ambil campaign
            } else {
            ...
            if (protocols.length === 0) {
                ...
                console.log(chalk.yellow(' ┊ └──'));
                partialFailure = true; // + tandai sebagai parsial jika tidak bisa ambil protocols
            } else {
            ...
            currentAccountXp = userInfo.xp; // + Mengambil nilai XP terbaru dari API Zealy
            ...
            // + Tentukan status akhir akun untuk laporan Telegram
            accountStatusText = partialFailure ? 'Parsial/Gagal' : 'Sukses Penuh';

            // + Tambahkan data akun ke reportData
            reportData.push({
                name: userInfo.name || 'N/A', // + Gunakan nama dari userInfo
                address: account.address,
                xp: currentAccountXp, // + Gunakan XP yang baru diambil
            ...
            failCount++;
            accountStatusText = 'Gagal Total'; // + Tandai sebagai gagal total jika ada exception

            // + Jika terjadi error fatal, tambahkan ke reportData dengan status gagal
            reportData.push({
                name: 'N/A', // + Nama mungkin tidak tersedia jika error di awal
                address: account.address,
                xp: 'N/A', // + XP pasti N/A jika error fatal
                status: accountStatusText
            });
        }

        if (accountSuccess && !partialFailure) { // + Hanya hitung sukses penuh
            successCount++;
        } else { // + Hitung semua yang tidak sukses penuh sebagai gagal/parsial
            failCount++;
        }
        console.log(chalk.gray(' ┊ ══════════════════════════════════════'));
        await sleep(2000); // + Tambahkan sedikit jeda antar akun
    }

    displayHeader(`═════[ Selesai @ ${getTimestamp()} ]═════`, chalk.blue);
    console.log(chalk.gray(` ┊ ✅ ${successCount} akun sukses, ❌ ${failCount} akun gagal/parsial`)); // + Ubah teks ini
    if (failedChats > 0) {
    ...
    // + Panggil sendTelegramReport dengan reportData yang sudah terisi
    console.log(chalk.magenta('DEBUG: sendTelegramReport dipanggil. reportData:', JSON.stringify(reportData, null, 2))); // Debugging final sebelum kirim
